...
public class QueryBuilder {
	...
	public QueryBuilder(ResourceRequest resourceRequest, QueryParams queryParams) {
		_queryParams = queryParams;
		_resourceRequest = resourceRequest;
	}
	
	/**
	 * Build the final query to be sent to the search backend
	 * 
	 * @param queryParams
	 * @return
	...
	protected void buildClassesCondition() throws ParseException {
	 ...
	 * @throws ParseException 
	 */
	protected void buildGroupsCondition() throws ParseException {
		...
		for (long l : groupIds) {
			query.addTerm(Field.SCOPE_GROUP_ID, String.valueOf(l), false, BooleanClauseOccur.SHOULD);				
		}
	 ...
	 * @throws ParseException
	 */
	protected void buildModificationTimeCondition() throws ParseException {
		...
		if (from != null) {
			BooleanQuery query = new BooleanQueryImpl();
			query.addRangeTerm("modified_sortable", from.getTime(), Long.MAX_VALUE);                      
			_query.add(query, BooleanClauseOccur.MUST);
		}
		...
		if (to != null) {
			BooleanQuery query = new BooleanQueryImpl();
			query.addRangeTerm("modified_sortable", to.getTime(), Long.MAX_VALUE);                      
			_query.add(query, BooleanClauseOccur.MUST);
		}
	}
	 ...
	 * @throws Exception 
	 */
	protected void buildViewPermissionCondition() throws Exception {
		...
		if (PortalUtil.isCompanyAdmin(user)) {
			return;
		}
		
		Role guestRole = RoleLocalServiceUtil.getRole(_queryParams.getCompanyId(), RoleConstants.GUEST);		
		Role siteMemberRole =  RoleLocalServiceUtil.getRole(_queryParams.getCompanyId(), RoleConstants.SITE_MEMBER);
		...
		if (themeDisplay.isSignedIn()) {
			query.addTerm(Field.ROLE_ID,  String.valueOf(guestRole.getRoleId()), false, BooleanClauseOccur.SHOULD);				
		}
		
		for (Role r : RoleLocalServiceUtil.getUserRoles(userId)) {
			
			if(_log.isInfoEnabled()) {
				_log.info("Regular role " + r.getName() + "(" + r.getRoleId() + ")");
			}
			query.addTerm(Field.ROLE_ID,  String.valueOf(r.getRoleId()), false, BooleanClauseOccur.SHOULD);				
		}		
		
		// Group roles

		// Notice that user.getGroupIds() won't give you groups joined by usergroup (Site Member Role)
		// Group returned by getSiteGroups() means implicity being in a "Site Member" role.
		
		for (Group g: user.getSiteGroups()) {
			
			long l  = g.getGroupId();
			
			_log.info("Group " + g.getName(_queryParams.getLocale()) + ": Site Member");

			query.addTerm(Field.GROUP_ROLE_ID, l + "-" + siteMemberRole.getRoleId(), false, BooleanClauseOccur.SHOULD);				

			for (Role r : RoleLocalServiceUtil.getUserGroupRoles(userId, l)) {

				query.addTerm(Field.GROUP_ROLE_ID,  l + "-" + r.getRoleId(), false, BooleanClauseOccur.SHOULD);				

				_log.info("Group " + g.getName(_queryParams.getLocale()) + ": Role " + r.getName() + "(" + r.getRoleId() + ")");
			}
		}
	...
	/**
	 * Get a condition to limit displaydate only to visible now.
	 * 
	 * @return
	 * @throws ParseException
	 */
	protected BooleanQuery getJournalArticleDisplayDateCondition() throws ParseException {
	...
	public static final DateFormat INDEX_DATE_FORMAT = new SimpleDateFormat("yyyyMMddHHmmss");
	private BooleanQuery _query;
	private QueryParams _queryParams;
	private ResourceRequest _resourceRequest;

	private static final Log _log =
					LogFactoryUtil.getLog(QueryBuilder.class);
}
