...
@Component(
	immediate = true, 
	property = {
		"menu.name=asset-types"
	},
	service = MenuOptionProvider.class
)
public class AssetTypeMenuOptionProvider implements MenuOptionProvider {
	...
	public void setOptions(PortletRequest portletRequest)
		...
		String[] configuration =
			_coreConfigurationHelper.getAssetTypeConfiguration();
		...
		for (int i = 0; i < configuration.length; i++) {
			...
			item.put(
				...
				_localizationHelper.getLocalization(
					locale, "option." +
						item.getString("entry_class_name").toLowerCase()));
			options.put(item);
		}
		
		template.put(GSearchWebKeys.ASSET_TYPE_OPTIONS, options);
	}
